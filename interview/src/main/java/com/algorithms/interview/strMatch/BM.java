...
public class BM {
     ...
     * @param sub
     */
    private static void buildBadCharPos(String sub) {
        for (int j = 0; j < 256; j++) {
            bad[j] = -1;
        }
     ...
     */
    private static void buildSuffixPrefix(String sub) {
        int i = 0;
        int j = 0;
        int len = 0;
    ...
    /**
     * @param j sub字符串和main字符串从后往前匹配的时候，在sub[j]位置与main匹配失败
     *          也就是说: sub[j+1,...,n)都和main字符串匹配成功了，是一个好后缀
     * @param n sub字符串的长度
     * @return 依次使用a), b), c)返回相应的值
     */
    private static int moveBySuffixPrefix(int j, int n) {
        // 因为已经匹配的位置是sub[j+1,n)
        // len表示已经匹配的字符串的长度
        int len = n - (j + 1);
    ...
    public static int strStr(String main, String sub) {
        ...
        while (i <= mainLen - subLen) {
            ...
            int badMoveLength = j - bad[main.charAt(i + j)];
